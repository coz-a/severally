...
both" — that is exactly the two peers above, `codex` and `antigravity`, and not yourself.
...
third member, `"claude-code:<model>"`, and read its caveat like any other. Send **one** `targets` call
