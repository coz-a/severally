...
`targets: ["claude-code", "antigravity", "opencode"]` — not just one. Then check your own
tier: if you are not yourself running as your default model (`gpt-6-astra`), your own lineage's top
tier, add `"codex"` to that same fan-out so it gets a peer too — a stronger model of your own
