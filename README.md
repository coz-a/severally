...
上は読み終えた瞬間に安心できる。下は、**その「一致」が根拠のある指摘と根拠のない断定の同居だった**ことが
一目で分かる。前者を選ぶと、気付く機会そのものが消える — 統合された後では、二人が同じ理由で同意したのか、
片方が何も見ずに頷いたのかを、もう区別できないからだ。

そして厄介なのは、**セカンドオピニオンが最も必要な場面は、都合のいい答えを最も受け入れやすい場面**でもある
ことだ。急いでいて、自分の案に自信があって、誰かに背中を押してほしいとき。そこで「おおむね一致」と言われて
断る人はいない。
...
どれも「役に立つ回答」の顔をしている。怖いのは**間違った答えが返ること**ではなく、**独立していない答えが
独立して見えること**だ。同意も、沈黙も、根拠のない断定も、外形は区別がつかない。

独立性は一度に失われるのではなく、気付かれずに少しずつ削られる — こちらの案を見せた時点で、相手が
こちらの文脈を読んだ時点で、ツールが「両者は概ね一致」と要約した時点で。**壊れ方を一つずつ数え上げ、
それぞれに対策を置いた**のがこのサーバである（→ §0）。

## 核にある一つの設計判断

相談相手は、こちらが書いた**ブリーフしか知らない**。リポジトリも、セッションの履歴も、こちらが何を試して
何を捨てたかも見えない。これは実装上の制約ではなく、このツールが選んだ一点の設計判断で、他のすべては
ここから出てくる。

> **書いたことは疑ってもらえる。書かなかったことは疑ってもらえない。**

前半が価値の源泉である。相談相手はこちらの枠組み — 問いの立て方、選択肢の切り方、「ここは問題ではない」と
決めた箇所 — を共有していない。だから、こちらが自明だと思って書いた前提にも、相手は前提として向き合う。
実際、返ってくる `findings` の `grounds` はブリーフの引用になる（「Established facts の『…』より」）。
相手が拠り所にできる事実は、そこにしかないからだ。

後半が価値の限界である。ブリーフに何を書くかを決めるのはこちらの枠組みなので、書き漏らした前提や、
仮説を事実の顔で書いた箇所は、相手からは見えない。相手はその閉じた世界の中で、整合的で自信のある答えを
返す。**このツールが守ってくれるのは自分の枠組みからであって、自分の事実からではない。** そしてこの二つは
完全には分離できない — 事実の取捨選択をしているのも枠組みだからだ。

だからこのサーバの機構は、次の三つのどれかに仕える。

1. **チャネルをブリーフ一本に保つ** — 隔離、既存セッションへの非接続、同一ブリーフの配布、再帰の禁止。
   これが崩れると前半が崩れる
2. **書かせる圧力をかける** — `explore` での案の拒否、`success_criteria`、`facts`。ブリーフを書く行為
   そのものが、自分の前提を言語化する行為になる
3. **答えがブリーフのどこに依存したかを追跡可能にする** — finding ごとの `grounds`、`unknowns`、
   `decision_changers`、`evidence_basis`。後半の代償は消せないので、遡れるようにする

そして**収束を代行しない**のは好みではなく、この構造の帰結である。「どの finding が正しいか」を決めるには、
その finding が依存した事実をリポジトリと照合しなければならない。それができるのは主担当だけだ。文脈の
少ない側に収束させれば、上の「後半」をサーバの内側にもう一段作ることになる。

### 何と比べればよいか

主担当エージェントの間違い方は二種類ある。

| | 枠組みの誤り | 事実の誤り |
|---|---|---|
| 例 | 問いの立て方、選択肢の切り方、「ここは問題ない」と決めた箇所 | バージョン、既存の挙動、テストが何を検証しているか |
| 直し方 | 枠組みを共有していない相手に見せる | リポジトリを見る |
| 文脈を渡すと | **伝染する** | 直る |

peer-consult は前者を狙い、後者を捨てる。[zen-mcp-server](https://github.com/BeehiveInnovations/zen-mcp-server)
は逆側に振っている — 相談相手の CLI をサンドボックスなしで起動し（作業ツリーを読めるかは設定と CLI の裁量に
よる）、会話の継続 ID で文脈を持ち越せる。事実は確認できる。その代わり、枠組みを共有する。
...
- **答えが 1 つ欲しいとき** — 統合も多数決もしない。食い違いは食い違いのまま返る
- **機械的に分岐したいとき** — yes/no の判定は返らない。読んで判断する人（または主担当エージェント）が要る
- **作業を任せたいとき** — 相談相手はコードを書かないしファイルも触らない

これらが欲しい場合は [zen-mcp-server](https://github.com/BeehiveInnovations/zen-mcp-server) のような
統合型が向いている。**優劣ではなく、どちらの失敗モードを狙うかの違い**である（上の表、および §0「移譲の線を
どこに引いたか」）。
...
冒頭に挙げた 4 つの経験は、この表の最初の 4 行に対応する。機能表ではなく、この対応表がこのプロジェクトの
中身である。各行の対策は、上の三つの目的 — チャネルを一本に保つ／書かせる／依存先を追跡可能にする — の
どれかに属する。

| 独立性が壊れる経路 | この実装の対策 |
|---|---|
| **自分の案を見せてから聞く**（アンカリング） | `explore` の初回は `context.proposal` を**拒否**する（`explore_proposal_not_allowed`）。案を批評させたいなら `review` を明示的に選ばせる |
| **ツールが合意を作る** | 複数に聞いても、サーバは一致・不一致を**判定しない**。機械的な横並びと「要約が似ていることは合意の証拠ではない」という注記だけを返す。各相談相手には `stance`（proceed / do_not_proceed / alternative / undetermined）を**自己申告**させ、そのまま横に並べる — 同じ finding を挙げながら結論が正反対、という食い違いが要約の読み方で消えないように |
...
| **同じ頭に二度聞く** | 呼び出し元と同じベンダーを指名した場合、結果に注記する — 新しいセッションが消すのは**セッション由来**の劣化（履歴、サンクコスト、自分の枠組みへの傾き）で、**系統由来**の盲点は残る、と。「他を優先せよ」とは書かない。実機で試すと、それは「この相手の findings は捨ててよい」と読まれた（[実験記録](docs/experiments/2026-09-10-fanout-reading.md)） |
