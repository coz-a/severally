...
エージェントが「進めていいか」と聞いてきたとき、こちらの経緯を知らない別の CLI（Codex、Claude Code、
Antigravity (Gemini) のいずれか）に案を読ませて反証を出させ、判断を左右する条件を主担当がリポジトリで確かめて
から人に返す、MCP サーバと Skill。
...
別のターミナルに手で貼っても近いことはできる。違うのは、**相手に渡るものがこちらの書いたブリーフだけである
ことを実行環境で強制する**点。相手は毎回新しい子セッションで、会話の履歴も捨てた案も届かない。`explore` は
自分の案が入っていると受け付けない。手元の実験では、案を伏せた回だけが、案を見せた回の出さない問題設定を
出した（2 組とも）。そのうえで、

- 返答が相手ごとに検算できる形で揃う。結論・指摘と根拠・足りなかった情報・判断が変わる条件・検証案・
  失敗の種類。複数に聞いても「おおむね一致」にまとめず、そのまま並べる
...
相談は毎回**専用の子セッション**として起動し（既存セッションには接続しない）、相手に渡るのはブリーフだけ。
1 回の依頼で最大 3 者に**同一のブリーフ**を投げて 1 つの `group_id` で受け取れるが、一致しているかどうかを
サーバが判定することはない。相手に許すことと止めることは「使う前に知っておくこと」にまとめた。
