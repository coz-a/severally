...
相談相手は、主担当が書いたブリーフしか知らない。リポジトリも、セッションの履歴も見えない。だから
返ってくるのは、ブリーフに書いた前提への反証候補と、確かめるべきこと、判断が変わる条件だ。
それをリポジトリで確かめて決めるのは主担当で、サーバは複数の答えを「おおむね一致」に統合しない。
回答者ごとの結論・根拠・不足情報・失敗の種別を、そのまま返す。
...
| | |
|---|---|
| [docs/concept.md](docs/concept.md) | なぜブリーフしか渡さないのか、なぜ一致を判定しないのか、何と比べればよいか、いつ使うか |
