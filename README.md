...
**多くの相談ツールは「答え」を返す。peer-consult は「答えを疑うための材料」を返す。**
...
一目で分かる。統合された後では、二人が同じ理由で同意したのか、片方が何も見ずに頷いたのかを、
もう区別できない。
...
**前半が価値の源泉。** 相談相手は主担当の枠組み — 問いの立て方、選択肢の切り方、「ここは問題ではない」と
決めた箇所 — を共有していない。主担当が自明だと思って書いた前提にも、相手は前提として向き合う。実際、
返ってくる `findings` の `grounds` はブリーフの引用になる（「Established facts の『…』より」）。相手が
拠り所にできる事実は、そこにしかない。
...
**収束を代行しない**のも、好みではなくこの帰結である。「どの finding が正しいか」を決めるには、それが
依存した事実をリポジトリと照合しなければならず、それができるのは主担当だけだ。文脈の少ないサーバに
収束させれば、後半の問題をもう一段作ることになる。
...
peer-consult は前者を狙い、後者を捨てる。[zen-mcp-server](https://github.com/BeehiveInnovations/zen-mcp-server)
は逆側に振っている — 相談相手の CLI をサンドボックスなしで起動し（作業ツリーを読めるかは設定と CLI の裁量
による）、会話の継続 ID で文脈を持ち越せる。事実は確認できるが、枠組みを共有する。

**自分の推論が疑わしいなら peer-consult、自分の把握が疑わしいなら zen。** 上位互換ではなく、別の失敗モード
への対策である。そして線を狭く引くことは機能追加では得られない — **収束できるツールは、「合意を作らない」
ことを同時に保証できない。** 保証とは「できないこと」で定義される。
