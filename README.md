...
受け取って終わりにしない。判断を左右する指摘を 1 件、相談した側のエージェントが自分のリポジトリで
確かめ、確かめた結果と、まだ確かめていない項目を添えて、あらためて人に「進めていいか」と聞く。
送ったブリーフ・相手の回答・指摘ごとの検証結果は 1 つの記録に揃い、Markdown にしてリポジトリに置ける。
...
- **返答が相手ごとに同じ形式で届く。** 結論・指摘と根拠・足りなかった情報・判断が変わる条件・検証案・
  失敗の種類。複数に聞いても要約されず、相手ごとにそのまま並ぶ
...
3 つの CLI のどれからでも残りの 2 つに同じ形で聞ける（3 起点とも実機で完走を確認済み。ただし日常の
運用実績は Claude Code 起点が 65 ラウンド、Codex と Antigravity 起点は各 1 ラウンド）。
...
| 相談相手 | シェル | ディスクの読み取り | 書き込み・実行 |
|---|---|---|---|
| Codex | read-only で持つ | できる | 不可 |
| Claude Code | 持たない | 読み取りツールなし | 不可 |
| Antigravity | 持たない | 読み取りツールなし | 不可 |

どの相手も空の作業ディレクトリで起動するのでリポジトリの場所は知らず、見せたい物はブリーフに貼る。
