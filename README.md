...
**別のモデルを呼ぶこと自体は難しくない。難しいのは、返ってきたものが独立した意見であり続けることだ。**
このサーバは、独立性が壊れる経路を一つずつ塞ぐために作られている（→ §0）。相談は毎回**専用の子セッション**
として起動し（既存セッションには接続しない）、相談相手には **Web 検索・閲覧のみ**を許可して、ファイル変更・
コマンド実行・さらなる相談は実行環境レベルで禁止する。1 回の依頼で最大 3 者（`targets: [...]`）に
**同一のブリーフ**を投げ、1 つの `group_id` でまとめて受け取ることもできる — ただし一致しているかどうかを
サーバが判定することはない。
...
## 0. コンセプト: 独立性は放っておくと壊れる

別のモデルを呼ぶことは難しくない。難しいのは、返ってきたものが**本当に独立した意見であり続ける**ことだ。
独立性は一度に失われるのではなく、気付かれずに少しずつ削られる — こちらの案を見せた時点で、相手が
こちらの文脈を読んだ時点で、ツールが「だいたい同じことを言っています」と要約した時点で。そして削られた
ことは結果からは見えない。**同意も、沈黙も、根拠のない断定も、外形はどれも「有用な回答」に見える。**

このサーバの設計は、ほぼすべてがこの一点に費やされている。壊れ方を一つずつ数え上げ、それぞれに対策を
置いた。機能表ではなく、この対応表がこのプロジェクトの中身である。
...
のような「議論させて収束させる」設計のものが向いている。こちらは逆方向、**収束させないこと**に価値を置いている。
