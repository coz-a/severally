...
生成される雛形は各 target が空のオブジェクトで、**何も上書きしていないことが一目で分かる**形になっている。
末尾の `//examples` に代表的な書き方が 3 つ入っているだけで、`//` で始まるキーはサーバが無視する。
検出結果（どの CLI が見つかったか、既定モデルは何か）はファイルではなく実行時の標準出力に出る。
