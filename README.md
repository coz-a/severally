...
**巻き戻しにくい判断の前に、自分の推論を、文脈を共有しない相手にさらす。返ってくるのは結論ではなく、
「何を確かめれば判断が変わるか」である。**

役割は二つに分かれる。相談相手はブリーフから**反証候補**を出す — 見落とした前提、反例、判断が変わる条件。
主担当はそれを**リポジトリで決着させる**。サーバはその間にあって、判断を代行せず、判断に要る材料を
失わせないためにある。

同じ問いを 2 者に投げたとき、主担当の手元に残るもの:

```
── peer-consult ────────────────────────────────
codex        stance: do_not_proceed
             「リトライが上限なしで増幅する」
             grounds: 添付コードに上限がない / confidence: medium / 参照 2 件
             next_check: 下流を落として再送回数を計測。線形なら杞憂
antigravity  stance: proceed
             「特に問題なし」
             grounds: なし / evidence_basis: thin / 参照 0 件
             quality.findings_without_grounds: 1

一致しているかどうかは、このサーバは判定しません。
```

主担当が次にすることは決まっている — 下流を落として再送回数を測る。「上限がない」は添付したコードに
ついての主張であって、リポジトリ全体についての主張ではない。確かめるのは主担当だ。antigravity の
「問題なし」は根拠がゼロだと分かるので、安心材料には数えない。

これが「両者はおおむね一致しています」と要約されていたら、根拠のある指摘と根拠のない断定は同じ重さに
潰れ、次に何を確かめればよいかも消えていた。厄介なのは、**セカンドオピニオンが最も必要な場面は、
都合のいい答えを最も受け入れやすい場面**でもあることだ。急いでいて、自分の案に自信があって、背中を
押してほしいとき、「おおむね一致」と言われて断る人はいない。
...
どれも「役に立つ回答」の顔をしている。怖いのは間違った答えが返ることではなく、**独立していない答えが
独立して見える**ことだ。同意も、沈黙も、根拠のない断定も、外形では区別がつかない。

## 役割分担 — 相手が出すもの、主担当が決めるもの

相談相手は、主担当が書いた**ブリーフしか知らない**。リポジトリも、セッションの履歴も、何を試して何を
捨てたかも見えない。これは実装上の制約ではなく選んだ設計判断で、役割の分かれ方はここから決まる。

**相談相手の役割は、ブリーフに対する反証候補を出すこと。** ブリーフに書いた問いの立て方、選択肢の
切り方、「ここは問題ではない」と決めた箇所は、相手にとって**書かれた前提**であって、自分が辿り着いた
前提ではない。主担当が自明だと思って書いたことにも、相手は前提として向き合う。返ってくるのは、
反例と、見落とした前提と、判断が変わる条件（`decision_changers`）と、確かめるべきこと（`next_checks`）
である。相手が継承しないのは主担当の**セッション**だ — 履歴も、サンクコストも、途中で捨てた案への
未練も届かない。

**主担当の役割は、それをリポジトリで決着させること。** 相手は添付した抜粋しか見ていないので、
「添付コードに上限がない」は抜粋についての主張でしかない。バージョン、既存の挙動、テストが何を
検証しているか — 事実はリポジトリを見られる側が確かめる。相手が要求して得られなかった情報は
`unknowns` として返るので、主担当がそこを埋める。

役割分担には限界がある。

> **書いたことは疑ってもらえる。書かなかったことは疑ってもらえない。**

何をブリーフに書くかを決めるのは主担当なので、書き漏らした前提や、仮説を事実の顔で書いた箇所は、
相手からは見えない。相手はその閉じた世界の中で、整合的で自信のある答えを返す。`grounds` がブリーフの
引用であることは、依存先が**辿れる**ということであって、**真である**ということではない。書き漏らしの
うち相手が気付いた分は `unknowns` に出てくるが、相手も気付かなかった分は出てこない。制約として
書いたことは、なおさら疑われにくい — 外から課された制約と、自分が「変えられない」と決めただけの
事柄は、分けて書く必要がある。

なお、履歴を渡さないことで判断が**改善する**かは、このプロジェクトの前提であって実測ではない。手元の
[実験](docs/experiments/2026-09-10-fanout-reading.md)が測ったのは主担当の**読み方**であり、隔離が判断を
改善するかは測っていない。決着させるなら、前提の罠を仕込んだ課題で、ブリーフのみの相談相手と
リポジトリ読み取り付きの相談相手を比べることになる。現時点で確認できている価値は、根拠・不足情報・
判断を覆す条件を、**回答者別に、失わずに**受け取れることだ。

サーバの機構は、ここから出てくる三つの目的のどれかに仕える。

1. **チャネルをブリーフ一本に保つ** — 隔離、既存セッションへの非接続、同一ブリーフの配布、再帰の禁止。
   崩れると相手は「書かれた前提」ではなく主担当の枠組みを継承する
2. **書かせる** — `explore` での案の拒否、`success_criteria`、`facts`。ブリーフを書くこと自体が、自分の
   前提を言語化することになる
3. **依存先を追跡可能にする** — finding ごとの `grounds`、`unknowns`、`decision_changers`、
   `evidence_basis`。「書かなかったことは疑われない」の代償は消せないので、遡れるようにする

## サーバが保証すること

サーバが返すのは**横並びまで**で、「この 2 つは同じことを言っている」という判定はしない。

保証するのは**保持**である。回答者ごとの `stance`、`findings` とその `grounds`、`unknowns`、
失敗の種別（`usage_limit` / `auth` / `timeout` …）を、要約で潰さずに返す。「両者はおおむね一致」の
一文に統合された時点で消えるのは、二人が同じ理由で同意したのか、片方が何も見ずに頷いたのか、
片方は一度も答えていないのか、という区別だ。

一致を判定しないのは、その判定が入力から機械的に導けないからだ（§0「移譲の線」）。「このジョブは
`usage_limit` で失敗した」「3 件の findings に `grounds` が無い」は導ける。「同じことを言っている」は
意味の解釈で、外れたときに主担当が原文に戻らない限り気付けない。横並びなら、その場で検証できる。

ただし横並びで十分というわけでもない。[実験](docs/experiments/2026-09-10-fanout-reading.md)では、
生のペイロードを読んだ主担当の 3/6 が、2 者が同じ finding から正反対の結論に至ったことを書き落とした。
内容は落ちない。落ちるのは**対比**だ。だから各相談相手に結論を `stance` として自己申告させ、機械的に
並べる。サーバができるのはそこまでで、読んで食い違いを拾うのは主担当の仕事として残る。

## 主担当が受け取るもの

相談の成果物は意見ではなく、**判断を安くする材料**である。結果のうち、主担当が実際に使うのは次の 4 つだ。

| フィールド | 主担当がすること |
|---|---|
| `next_checks` | 挙げられた検証をリポジトリで実行する。相手が見られない事実は、ここで主担当が拾う |
| `decision_changers` | 自分の判断が何に依存しているかを確認する。条件が成立していれば判断を変える |
| `unknowns` | 相手が要求して得られなかった情報。ブリーフの書き漏らしがここに現れる |
| `stance` と `comparison.by_target` | 複数に聞いたとき、どこが食い違ったかを自分で読む。似た要約は一致の証拠ではない |

相談の成果は指摘の数ではない。主担当が `findings` ごとに**採用・不採用・保留**を決め、`next_checks` の
うち何を実行したか、その結果として判断を**変えたのか維持したのか**を理由つきで記録して、相談は閉じる。
判断が変わらなかった場合も「確認できた」ことに価値があるので、変わった場合と分けて数える（§7）。

Skill はこれを主担当に課すが、**サーバはまだ記録しない**（§7 の指標は現状、手集計になる）。同じく、
finding がブリーフ上の**どの事実に依存したか**も自由記述の `grounds` から読み取るしかない。この二つは、
コンセプトが要求していて実装がまだ追いついていない箇所である。

## いつ使うか

相談は数分と、相手 CLI のクォータと、ブリーフを書く手間を使う。次の三つが揃うときに元が取れる。

1. **誤判断の手戻りが大きい** — 公開 API やスキーマの互換性、データ移行、並行・分散設計、障害時の
   再試行方針。既存のテストを一度回せば決まる変更は対象ではない
2. **判断に必要な前提をブリーフに書ける** — 目的、観測した事実、仮説、外から課された制約、自分で決めた
   だけの方針を、分けて書ける。書けないなら相手は閉じた世界で答えるだけになる
3. **返ってきた検証を主担当が実行できる** — `next_checks` をリポジトリで確かめる時間がある。確かめずに
   採用するなら、相談は指摘の数を増やしただけで終わる

三つ目の条件は見落としやすい。自分の案に確信があって反証が浮かばないときほど、相談の価値は高く、
返ってきた検証を飛ばしたくなる。

1 者で足りることが多い。複数に聞くのは独立した読みが 2 つ欲しいときで、その場合は**同じブリーフを
同時に配る** — 1 者の答えを見てから 2 者目に聞くと、ブリーフを書き直す誘惑が生まれる。同時配布は
その機会を消すためにある。上位互換ではなく、別の失敗モードへの対策である。

次が欲しいときは向いていない:

- **答えが 1 つ欲しい** — 統合も多数決もしない。食い違いは食い違いのまま返る
- **機械的に分岐したい** — yes/no は返らない。読んで判断する主担当が要る
- **作業を任せたい** — 相談相手はコードを書かないし、ファイルも触らない

## 何と比べればよいか

複数モデルに聞く MCP は他にもある。代表として
[zen-mcp-server](https://github.com/BeehiveInnovations/zen-mcp-server) と比べる（2026-09-10 時点の
`docs/tools/consensus.md` と `clink.md` による）。

「統合するかどうか」は、実は差ではない。zen-mcp の `consensus` はモデルを逐次呼び、統合は呼び出し側の
エージェント — つまり主担当自身 — が行う。peer-consult の横並びを読んで判断するのも主担当だ。
どちらも最後に判断するのは同じ頭で、差があるのは**その頭に何が渡るか**と、**何が主担当の都合で外せないか**
である。

| | zen-mcp-server | peer-consult |
|---|---|---|
| 相談相手に渡るもの | 呼び出し側が選んだファイル参照。`clink` では相手 CLI が自分で読む範囲を決める | ブリーフのみ。ローカルファイルは読めない |
| 文脈の持ち越し | `continuation_id` で前回の会話を継続できる（任意） | 毎回新しい子セッション。継続はしない |
| 相手 CLI の権限 | 既定でサンドボックスと承認を外して起動（`--dangerously-bypass-approvals-and-sandbox` 等）。フラグは外せる | 編集・実行・MCP・再帰相談を実行環境レベルで剥奪。リクエストからは緩められない |
| 複数に聞くとき | 逐次。賛成・反対の立場を指定できる | 同一ブリーフを同時配布。立場の指定はない |
| 返るもの | 各モデルの意見。統合は呼び出し側 | 回答者別の `stance` / `findings` / `grounds` / 失敗種別。統合はしない |

zen-mcp を「新しい会話で、ファイルを渡さず、統合せずに」使えば、peer-consult に近い運用はできる。
違いは、それが**既定**か**選択**かだ。締切に追われれば「今回だけ文脈を渡す」と言い出す。その圧力に
晒されない場所に置くものと、主担当に残すものの線引きが §0 で、peer-consult はその線を実行環境の
側に引いている。

逆に、相談相手にリポジトリを読ませたい、会話を続けたい、作業までさせたいなら、zen-mcp の方が向いている。
peer-consult はそれらを**できなくすること**で成り立っているので、機能追加では両立しない。
...
独立性は一度に失われるのではなく、気付かれずに少しずつ削られる — 案を見せた時点で、相手が文脈を読んだ
時点で、ツールが「概ね一致」と要約した時点で。壊れ方を一つずつ数え上げ、それぞれに対策を置いたのが
この表で、機能表ではなくこれがこのプロジェクトの中身である。冒頭の 4 つの経験は最初の 4 行に対応し、
各行の対策は上の三つの目的のどれかに属する。

| 独立性が壊れる経路 | この実装の対策 |
|---|---|
| **自分の案を見せてから聞く**（アンカリング） | `explore` の初回は `context.proposal` を**拒否**する（`explore_proposal_not_allowed`）。案を批評させたいなら `review` を明示的に選ばせる |
| **ツールが合意を作る** | 複数に聞いても、サーバは一致・不一致を**判定しない**。機械的な横並びと「要約が似ていることは合意の証拠ではない」という注記を返す。各相談相手の結論は `stance`（proceed / do_not_proceed / alternative / undetermined）として**自己申告**させ、そのまま並べる — 同じ finding から正反対の結論、という食い違いが要約の読み方で消えないように |
...
| **相手がこちらの文脈を読む** | 相談相手はローカルファイルを読めず、主担当のセッションも見えない。渡るのはブリーフだけ。毎回新しい子セッションで、既存セッションには接続しない |
...
| **同じ頭に二度聞く** | 主担当と同じベンダーを指名した場合、結果に注記する — 新しいセッションが消すのは**セッション由来**の劣化（履歴、サンクコスト、枠組みへの傾き）で、**系統由来**の盲点は残る、と。「他を優先せよ」とは書かない。実機ではそれが「この相手の findings は捨ててよい」と読まれた（[実験記録](docs/experiments/2026-09-10-fanout-reading.md)） |
...
「何をツールに置き、何を主担当に残すか」は、3 つの問いで決めている。

1. **主担当の都合で曲げられてはいけないか。** 締切に追われれば「今回だけ」と言い出す。その圧力に晒されない
   場所がツールである。権限の剥奪、再帰の禁止、ラウンド上限、秘密の除去、アンカリング拒否はすべてここ —
   skill の「お願い」として書いてあるだけなら、忙しいときに飛ばされる
2. **ツールの持つ情報だけで正しくできるか。** 統合、「この根拠で十分か」、「この相手はこの領域に強いか」は
   リポジトリ・経緯・意図を要する。文脈がある場所で判断すべきで、それは主担当である
3. **間違えたとき、主担当は気付けるか。** 「両者は一致」と返されたら、本物の一致か作られた一致かは後から
   見分けられない。「メンバー 2 件、うち 1 件は `usage_limit` で失敗」なら、その場で検証できる。
...
「このジョブは `usage_limit` で失敗した」「3 件の findings に `grounds` が無い」「主担当と同じベンダーだ」は
導ける。「この 2 つは同じことを言っている」は導けない。だから返すのは横並びまでで、一致の判定はしない。

逆に、移譲を減らせば安全というわけでもない。不変でなければならないものを主担当に任せれば、保証は最初に
忙しくなった日に消える。狙うのは**判断を代行せず、判断を安くする**こと — `comparison.by_target`、
`grounds` の欠落件数、`decision_changers`、`next_checks` は、結論を出さずに、結論を出す作業を軽くするために
ある。
...
consult_get({ group_id, wait_ms? }) -> fan-out の状態／結果を1回でまとめて取得。`members`（各 target の状態・結果）、
                             `comparison.by_target`（機械的な横並び。各相談相手が自己申告した `stance` を先頭に置く。
                             サーバは一致しているかどうかを判定しない）、
...
| `caller` | – | 呼び出し元 CLI（同じ表記が使える）。`target`/`targets` と同じベンダーだと `quality.caveat` に「フレッシュコンテキストでの再読であって独立した意見ではない — セッション由来の劣化は消え、系統由来の盲点は残る」と注記される |
...
summary                 見解の要約
confidence              全体の確信度
stance                  proceed / do_not_proceed / alternative / undetermined（相談相手の自己申告。
                        fan-out の comparison.by_target で先頭に並ぶ）
...
```json
{
  "targets": {
    "codex":       { "enabled": false },
    "claude-code": { "allowed_models": ["claude-opus-5", "claude-sonnet-5"] },
