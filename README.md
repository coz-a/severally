...
同じ問いを 2 者に投げたとき、返ってくるものの違い:

```
── 統合するツール ──────────────────────────────
両者はおおむね一致しています。このアプローチで問題ないでしょう。
```
...
上は読んだ瞬間に安心できる。下は、その「一致」が**根拠のある指摘と根拠のない断定の同居**だったことが
一目で分かり、次に何を確かめればよいかまで書いてある。統合された後では、二人が同じ理由で同意したのか、
片方が何も見ずに頷いたのかを、もう区別できない。

厄介なのは、**セカンドオピニオンが最も必要な場面は、都合のいい答えを最も受け入れやすい場面**でもある
ことだ。急いでいて、自分の案に自信があって、背中を押してほしいとき、「おおむね一致」と言われて断る人は
いない。
...
捨てたかも見えない。これは実装上の制約ではなく選んだ設計判断で、他のすべてはここから出てくる。

> **書いたことは疑ってもらえる。書かなかったことは疑ってもらえない。**

**前半が価値の源泉。** 相談相手が継承しないのは主担当の**セッション**である。履歴も、サンクコストも、
途中で捨てた案への未練も届かない。ブリーフに書いた枠組み — 問いの立て方、選択肢の切り方、「ここは
問題ではない」と決めた箇所 — は届くが、相手にとってそれは**書かれた前提**であって、自分が辿り着いた
前提ではない。主担当が自明だと思って書いたことにも、相手は前提として向き合う。

**後半が価値の限界。** 何をブリーフに書くかを決めるのは主担当の枠組みなので、書き漏らした前提や、仮説を
事実の顔で書いた箇所は、相手からは見えない。相手はその閉じた世界の中で、整合的で自信のある答えを返す。
`grounds` がブリーフの引用であることは、依存先が**辿れる**ということであって、**真である**ということでは
ない。書き漏らしのうち相手が気付いた分は `unknowns` に出てくるが、相手も気付かなかった分は出てこない。
制約として書いたことは、なおさら疑われにくい — 外から課された制約と、自分が「変えられない」と決めた
だけの事柄は、分けて書く必要がある。
**このツールが守るのは自分の枠組みからであって、自分の事実からではない。** そして二つは分離しきれない —
事実の取捨選択をしているのも枠組みだからだ。
...
3. **依存先を追跡可能にする** — finding ごとの `grounds`、`unknowns`、`decision_changers`、
   `evidence_basis`。後半の代償は消せないので、遡れるようにする

**収束を代行しない**のも、好みではなくこの帰結である。ただし理由は「リポジトリを見ないと決められない
から」ではない — 論理や優先順位の食い違いは、リポジトリを見ても決まらない。理由は、「この 2 つは同じことを
言っている」という判定が、外れたときに**主担当が後から気付けない主張**だからだ（§0「移譲の線」）。
ツールが返してよいのは横並びまでで、一致の判定は文脈のある側に残す。
...
相談は、主担当が `findings` ごとに**採用・不採用・保留**を決め、その理由を記録して閉じる。Skill は
これを主担当に課すが、**サーバはまだ記録しない**（§7 の指標は現状、手集計になる）。同じく、finding が
ブリーフ上の**どの事実に依存したか**も自由記述の `grounds` から読み取るしかない。この二つは、
...
| | 枠組みの誤り | 事実の誤り |
|---|---|---|
| 例 | 問いの立て方、選択肢の切り方、「ここは問題ない」と決めた箇所 | バージョン、既存の挙動、テストが何を検証しているか |
| 直し方 | 枠組みを共有しない相手に見せる | リポジトリを見る |
| 文脈を渡すと | **伝染する** | 直る |

peer-consult は前者を狙い、後者は**主担当に返す** — 相手が挙げた `next_checks` と `unknowns` を、
リポジトリを見られる側が確かめる。[zen-mcp-server](https://github.com/BeehiveInnovations/zen-mcp-server)
は逆側に振っている — 相談相手の CLI をサンドボックスなしで起動し（作業ツリーを読めるかは設定と CLI の裁量
による）、会話の継続 ID で文脈を持ち越せる。事実は確認できるが、枠組みを共有する。

どちらの誤りをしているかは、事前には分からないことが多い。だから使いどころは誤りの種類ではなく
**判断の重さ**で決める。1 者で足りることが多い。複数に聞くのは、独立した読みが 2 つ欲しいときで、それは
**同じブリーフを同時に配る**ことでしか得られない — 1 者の答えを見てから 2 者目に聞けば、2 者目の
ブリーフは既に 1 者目に染まっている。上位互換ではなく、別の失敗モードへの対策である。そして線を狭く
引くことは機能追加では得られない — **収束できるツールは、「合意を作らない」ことを同時に保証できない。**
保証とは「できないこと」で定義される。

なお「文脈を渡すと枠組みが伝染する」は、このプロジェクトの前提であって実測ではない。手元の
[実験](docs/experiments/2026-09-10-fanout-reading.md)が測ったのは主担当の**読み方**であり、隔離が判断を
改善するかは測っていない。決着させるなら、枠組みの罠を仕込んだ課題で、ブリーフのみの相談相手と
リポジトリ読み取り付きの相談相手を比べることになる。

したがって、次が欲しいときは統合型が向いている:
