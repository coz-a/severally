---
name: peer-consult
description: Use when a decision deserves a second, independent mind - an architectural or hard-to-reverse choice, two options that look genuinely close, or an investigation that has stalled - to get an independent opinion, review or structured debate from Claude Code through the peer-consult MCP server. Also use when the user asks for it directly ("ask Claude", "get Claude to review this", "Claudeに聞いて", "Claudeにレビューして", "second opinion", "セカンドオピニオン").
metadata:
  short-description: Ask Claude Code for an independent opinion, review or debate
---

# Consulting Claude Code

You have a peer: a fresh Claude Code session, reached through the `peer-consult` MCP server. It can search and
...
- **`failed`** — no advice was obtained. `failure.kind` says whether it was `timeout`, `auth`, `usage_limit`,
  ...
  do not present a failure as "Claude had no concerns".
